...
module.exports = async bot => {
  ...
  setInterval(async () => {
    ...
    guilds.forEach(async guild => {
      [...guild.planted].forEach(async greenhouse => {
        ...
        if ((Date.now() - greenhouse.time) >= 1800000) {
          ...
          bot.logger.log("Greenhouse removed from DB.");
      ...
      [...guild.reports].forEach(async report => {
        if ((Date.now() - report.time) < 1800000) return;

        guild.reports.splice(guild.reports.indexOf(report), 1);
        await bot.guildInfo.set(bot, {
